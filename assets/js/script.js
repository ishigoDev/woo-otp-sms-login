jQuery(document).ready(function ($) {
      ...
      success: function (response) {        
        if (response.status == 1) {
          ...
          if (response.gateway == 'Firebase') {
            ...
            jQuery(
              '<button type="button" id="resend_otp" disabled>Resend in <span id="timer">' +
                response.resend_timer_firebase +
                '</span></button>',
            ).insertAfter('#verify_otp')
            var firebaseConfig = {
            ...
            var phoneNumber =
              '+' + response.response_back.country_code + mobile_number
            ...
            firebase
              ...
              .catch(function (error) {
                if (error.code == 'auth/quota-exceeded') {
                  ...
                  too_many_error =
                    'Too many request for the OTP . Please Try Again Later.'
        ...
        if (response.status == 0) {
          $account_error =
            'An account is not registered.<a href="#" class="showregister">Please sign up.</a>'
        ...
        if (response.status == -2) {
          $error_message =
            'Error while sending otp.<a href="#" class="showregister">Please check the authorization key.</a>'
      ...
      complete: function (data) {           
        ...
        jQuery('#resend_otp').click(function () {
          ...
          jQuery.ajax({
            ...
            data: form_data,
            success: function (response) {
              if (response.gateway == 'Fast2Sms' || response.gateway == 'Twilio') {
                if (response.status == 1) {
                  sessionStorage.setItem('otp_token', response.otp)
                }
                if (response.status == 0) {
                ...
                if (response.status == -2) {
                  error_response(
                    ' While sending otp.Please check the authorization key.',
                  )
                }
              }
              if (response.gateway == 'Firebase') {
  ...
  jQuery('#verify_otp').click(function () {
    ...
    jQuery.ajax({
      ...
      data: form_data,
      beforeSend:function(){
        jQuery('#verify_otp').addClass('loading_verify');
        jQuery('#verify_otp').prop("disabled",true);
      },
      success: function (response) {
        if (response.status == 1) {
          if (response.gateway == 'Fast2Sms|Twilio') {
      ...
      complete:function(){
        jQuery('#verify_otp').prop("disabled",false);
        jQuery('#verify_otp').removeClass('loading_verify');
      }
    })
  })
  //Timer JS
  ...
  function error_response(error_message) {
    ...
    jQuery('.woocommerce .woocommerce-notices-wrapper').append(
