    ...
    class WOO_OTP_FRONTEND{
        ...
        public function woocom_extra_register_fields() {
            ...
            <p class='form-row form-row-wide'>
                <label for='reg_mobile_number'><?php _e( 'Mobile Number', 'woo-otp-login-sms' ); ?><span class='required'> *</span></label><input type='text' class='input-text' name='reg_mobile_number' id='reg_mobile_number' value='<?php if ( ! empty( $_POST['reg_mobile_number'] ) ) esc_attr_e( $_POST['reg_mobile_number'] ); ?>' />                
            </p>                
            <?php
        }
        public function woocom_validate_extra_register_fields( $username, $email, $validation_errors ){                
            if (isset($_POST['reg_mobile_number']) && empty($_POST['reg_mobile_number']) ) {
                $validation_errors->add('mobile_number_error', __('Mobile Number is Required!', 'woo-otp-login-sms'));
            }
            if (isset($_POST['reg_mobile_number']) && !empty($_POST['reg_mobile_number']) && $_POST['reg_mobile_number'] < 10  ) {
                $validation_errors->add('mobile_number_error', __('Please type a correct phone number!', 'woo-otp-login-sms'));
            }
            $user_exists = $this->wooc_get_users_by_phone($_POST['reg_mobile_number']);            
            if($user_exists){                
                $validation_errors->add('mobile_number_error', __('User already exists with this nummber!', 'woo-otp-login-sms'));
            }
