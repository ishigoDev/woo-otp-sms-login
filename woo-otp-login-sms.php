<?php
/**
 * Plugin Name: Woocommerce OTP Login via SMS
 * Description: This plugin help you login the user using OTP via SMS in the woocommerce authentication.
 * Version: 1.0
 * Author: Evince Development Pvt. Ltd.
 * Author URI: https://evincedev.com/
 * Text Domain: woo-otp-login-sms
 * */
...
if (!defined('WOO_OTP_DB_SETTING_TWILIO')) {
    define(
        'WOO_OTP_DB_SETTING_TWILIO',
        $wpdb->prefix . 'phone_otp_setting_twilio'
    );
}
...
if (
    ...
    if (!class_exists('WOO_OTP_VERIFICATION')) {
        class WOO_OTP_VERIFICATION
        {
            public function __construct()
            {
                ...
                register_activation_hook(WOO_OTP_PATH, 
                    new WOO_OTP_ACTIVATE()
                );                
                register_deactivation_hook(__FILE__, [
                    $this,
                    'my_plugin_remove_database',
                ]);
                ...
                function my_plugin_scripts()
                {
                    wp_enqueue_style(
                        'woo-otp-admin-style',
                        WOO_OTP_URI . '/assets/css/admin-style.css',
                        '',
                        1.2
                    );
                    wp_enqueue_script(
                        'woo-otp-admin-script',
                        WOO_OTP_URI . '/assets/js/admin/admin_script.js',
                        '',
                        1.2
                    );
                    wp_localize_script(
                ...
                function custom_add_js()
                {
                    wp_enqueue_style(
                        'woo-otp-style',
                        WOO_OTP_URI . '/assets/css/style.css',
                        '',
                        1.6
                    );
                    wp_enqueue_script(
                        'woo-otp-js',
                        WOO_OTP_URI . '/assets/js/script.js',
                        '',
                        1.0,
                        ['jquery']
                    );
                ...
                {
                    $settings_link =
                        '<a href="admin.php?page=woo-otp-login-sms-setting">' .
                        __('Settings', 'woo-otp-login-sms') .
                        '</a>';
    ...
    add_action('admin_notices', function () {
        echo '<div id="message" class="notice notice is-dismissible"><p>' .
            esc_html_e('Woocommerce', 'woo-otp-login-sms') .
            '<b>' .
            esc_html_e(
                'is not active. Please activate woocommerce first!.',
                'woo-otp-login-sms'
            ) .
            '</b></p></div>';
    });
